...
void loop()
{
  ...
  if (myIMU.dataAvailable() == true)
  {
    ...
    uint16_t eulerX_int = eulerX*1000;
    uint16_t eulerZ_int = eulerZ*1000;
    uint32_t data = uint32_t(eulerX_int*pow(2,16)+eulerZ_int);
    //data_old = data;
    memcpy(data_byte, &data, sizeof(data));
    Serial.println(fTime2-fTime1);
    //bleuart.write(data_byte,4);
    fTime1 = fTime2;
    memcpy(euler_byte, &eulerZ_int, sizeof(eulerZ_int));
    bleuart.write(euler_byte,2);
  }
}
